...
$course_check_query = "SELECT * FROM course WHERE c_id='$c_id'  LIMIT 1";
  ...
  if ($course) { // if course exists
     ...
     $c_faculty_id = $course['faculty_id'];
     $faculty_check_query = "SELECT * FROM faculty WHERE id='$c_faculty_id'  LIMIT 1";
     $result2 = mysqli_query($db, $faculty_check_query);
     $faculty = mysqli_fetch_assoc($result2);
     $c_faculty_name = $faculty['username'];
  }
?>
...
<div class="input-group">
        ...
        <input type="text" value="<?php echo $c_faculty_name?>" readonly>
</div>
<div class="input-group">
  	  <label>Faculty ID</label>
        <input type="text" value="<?php echo $c_faculty_id?>" readonly>
</div>
		<div class="input-group">
